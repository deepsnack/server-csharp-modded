...
function renderAddFields() {
    ...
    if (src === 'trader') {
        f.innerHTML = `<label>商人 ID</label><input id="f-traderId" placeholder="如 54cb50c76803fa8b248b4571（Prapor）" />
            ...
            <label>价格（每行 <code>tpl 数量</code>，留空=无限免费）</label><textarea id="f-cost" class="json-edit" style="min-height:54px"></textarea>`;
    } else if (src === 'quest') {
        ...
        f.innerHTML = `<label>角色</label><select id="f-role"><option value="output">产物（新建简易配方）</option><option value="ingredient">原料（加入现有配方）</option></select>
            ...
            <label>数量</label><input id="f-count" type="number" value="1" />
            <label>原料（产物模式：每行 <code>tpl 数量</code>）</label><textarea id="f-cost" class="json-edit" style="min-height:54px"></textarea>`;
    } else if (src === 'loot') {
...
function parseCost(text) {
    ...
    (text || '').split(/\r?\n/).forEach(line => {
        ...
        if (p.length >= 2 && isTpl(p[0])) { const c = parseInt(p[1], 10); if (c > 0) out.push({ tpl: p[0], count: c }); }
    });
    return out;
}
async function submitAdd(tpl) {
    ...
    if (src === 'trader') {
        ...
        ov.cost = parseCost(el('f-cost').value);
